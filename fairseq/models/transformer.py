...
class TransformerModel(FairseqModel):
    ...
    def build_model(cls, args, task):
        ...
        if args.share_all_embeddings:
            ...
            if tgt_in_dict is None:
                decoder_embed_tokens = encoder_embed_tokens
            else:
                decoder_embed_tokens = build_embedding(
                    tgt_in_dict, args.decoder_embed_dim, args.decoder_embed_path
                )
            ...
            decoder_embed_tokens = build_embedding(
                tgt_dict if tgt_in_dict is None else tgt_in_dict,
                args.decoder_embed_dim, args.decoder_embed_path
            )

        encoder = TransformerEncoder(args, src_dict, encoder_embed_tokens)
        decoder = TransformerDecoder(args, tgt_dict, decoder_embed_tokens, enc_embed_tokens=encoder_embed_tokens if tgt_in_dict and args.share_all_embeddings else None)
    ...
    def forward(self, src_tokens, src_lengths, prev_output_tokens,
        ...
        """
        encoder_out = self.encoder(src_tokens, src_lengths)
        decoder_out = self.decoder(prev_output_tokens, encoder_out, spans)
        return decoder_out
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, enc_embed_tokens=None, no_encoder_attn=False, left_pad=False, final_norm=True):
        super().__init__(dictionary)
        self.dropout = args.dropout
    ...
    def forward(self, prev_output_tokens, encoder_out=None,
                spans=None, incremental_state=None):
        """
        Args:
            prev_output_tokens (LongTensor): previous decoder outputs of shape
                `(batch, tgt_len)`, for input feeding/teacher forcing
        ...
        if self.adaptive_softmax is None:
            ...
            if spans is not None:
                bsz, span_seqlen = spans.shape
                _, seqlen, dim = x.shape

                base = torch.arange(bsz, device=spans.device) * seqlen
                base = base.unsqueeze(-1)

                expanded_idx = spans + base
                flat_idx = expanded_idx.view(-1)

                x = torch.index_select(x.contiguous().view(-1, dim), 0, flat_idx).view(bsz, span_seqlen, dim)
            # project back to size of vocabulary
            if self.share_input_output_embed:
                if self.enc_embed_tokens is None:
                    x = F.linear(x, self.embed_tokens.weight)
                else:
                    #print("enc_embed_tokens")
                    x = F.linear(x, self.enc_embed_tokens.weight)
            else:
