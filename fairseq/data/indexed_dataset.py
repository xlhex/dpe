...
def data_file_path(prefix_path, type=None):
    if type is None:
        return prefix_path + '.bin'
    else:
        return prefix_path + ".{}.bin".format(type)
...
class IndexedRawTextSegDataset(IndexedRawTextDataset):
    """Takes a text file as input, segment and binarizes it in memory at instantiation.
    Original lines are also kept in memory"""

    def __init__(self, path, dictionary, ex_dict=None, is_tgt=False, append_eos=True):
        self.tokens_list = []
        self.lines = []
        self.sizes = []
        self.append_eos = append_eos
        ...
        maxspan = 0
        if self.is_tgt:
            for s in self.dictionary.symbols[4:]:
                if not s.startswith("madeupword"):
                    maxspan = max(maxspan, len(s))
            self.pos_list = []
            self.segments_list = []
            self.lens_list = []
        self.maxspan = maxspan

        self.read_data(path, dictionary, ex_dict)
        #self.size = len(self.tokens_list)
    ...
    def read_data(self, path, dictionary, ex_dict):
        with open(path, 'r') as f:
            for i, line in enumerate(f):
                ...
                if self.is_tgt:
                    s = " ".join(["▁"+word for word in "".join(line.strip("\n").split()).replace("▁", " ").strip().split()])
                    self.lines.append(s)
                else:
                    self.lines.append(line.strip('\n'))
    ...
    def __getitem__(self, i):
        self.check_index(i)
        # tokenize target side
        if self.is_tgt:
            return Tokenizer.segment(self.lines[i], self.dictionary, self.ex_dict,
                                     self.is_tgt, self.maxspan, append_eos=self.append_eos)
        # tokenize source side
        else:
            return Tokenizer.segment(self.lines[i], self.dictionary, self.ex_dict,
                                     self.is_tgt, self.maxspan, append_eos=self.append_eos)[0]
